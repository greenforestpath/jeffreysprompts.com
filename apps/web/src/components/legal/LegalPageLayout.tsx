...
export function LegalPageLayout({
  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 print:bg-white">
      {/* Header */}
      <div className="border-b border-border/60 bg-white dark:bg-neutral-900 print:border-b-2 print:border-black">
      ...
      <div className="container-wide py-8 sm:py-12">
        <div className="grid gap-8 lg:grid-cols-[250px_1fr] xl:grid-cols-[280px_1fr]">
                ...
                <ul className="space-y-1">
                  {tableOfContents.map((item) => (
                    <li key={item.id}>
                      <a
                        href={`#${item.id}`}
                        className="block text-sm text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white py-1 transition-colors"
                      >
                        {item.title}
                      </a>
                    </li>
          ...
          <main className="min-w-0">
            {/* Mobile TOC */}
            <details className="lg:hidden mb-8 bg-white dark:bg-neutral-900 rounded-lg border border-border/60 print:hidden">
